...
export default function ManageCategoriesPage() {
  ...
  const handleDelete = async (id) => {
    ...
    try { await api(`/categories/${id}`, { method: 'DELETE' }); loadCategories(); }
          ...
          {categories.map((cat) => (
            <Card key={cat.id} className="flex items-center justify-between py-4">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="ghost" onClick={() => handleDelete(cat.id)} className="text-sm text-error hover:text-error">Delete</Button>
              </div>
            </Card>
          ))}
        </div>
