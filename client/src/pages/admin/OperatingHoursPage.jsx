  ...
  return (
    <div className="py-6 animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-text-primary">Operating Hours</h1>
      ...
      {message && (
        <div className={`p-3.5 rounded-xl mb-5 text-sm font-medium animate-slide-up ${message.startsWith('Error') ? 'bg-red-50 text-error' : 'bg-green-50 text-green-700'}`}>
          {message}
        </div>
      )}
      ...
      <div className="space-y-3">
        {hours.map((h) => (
          <Card key={h.dayOfWeek} className={`flex flex-col sm:flex-row sm:items-center gap-3 py-4 transition-opacity ${h.isClosed ? 'opacity-50' : ''}`}>
            <div className="font-semibold text-text-primary min-w-[100px]">{getDayName(h.dayOfWeek)}</div>
            <div className="flex items-center gap-3 flex-1 flex-wrap">
              <input
                type="time" value={h.openTime}
                onChange={(e) => updateHour(h.dayOfWeek, 'openTime', e.target.value)}
                disabled={h.isClosed}
                className="border border-border rounded-lg px-3 py-2 min-h-[44px] text-sm focus:ring-2 focus:ring-primary/50"
              />
              ...
              <label className="flex items-center gap-2 cursor-pointer min-h-[44px] ml-auto">
                ...
                />
                <span className="text-sm text-text-secondary font-medium">Closed</span>
              </label>
            </div>
          </Card>
        ))}
