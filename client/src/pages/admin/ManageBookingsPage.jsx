            ...
            <Card key={b.id} className="py-4">
              ...
              {(b.status === 'pending' || b.status === 'confirmed') && (
                <div className="flex items-center gap-2 mt-3 pt-3 border-t border-white/10">
                  ...
                  {/* WhatsApp button:
                        - Before Send Confirm clicked → "Send Confirm" (green)
                        - After Send Confirm clicked OR status=confirmed → "Send Reminder" (amber)
                        Always uses WhatsApp icon. */}
                  {b.customerPhone && (() => {
                    const alreadySent = confirmSent.has(b.id) || b.status === 'confirmed';
                    const kind = alreadySent ? 'reminder' : 'confirm';
                    const label = alreadySent ? 'Send Reminder' : 'Send Confirm';
                    const theme = alreadySent
                      ? 'bg-amber-500/20 text-amber-400 hover:bg-amber-500/30'
                      : 'bg-green-600/20 text-green-400 hover:bg-green-600/30';
                    return (
                      <button
                        onClick={() => sendWhatsApp(b, kind)}
                        className={`flex-1 min-w-0 text-xs font-medium px-3 py-2 rounded-lg min-h-[36px] transition-colors flex items-center justify-center gap-1.5 ${theme}`}
